...
def vision_screenshot(path: str = "screen.png", prompt: str = "Describe the visible screen and important UI controls.") -> str:
    ...
    response=OpenAI(api_key=key).chat.completions.create(model=MODEL,messages=[{"role":"user","content":[{"type":"text","text":prompt},{"type":"image_url","image_url":{"url":f"data:image/png;base64,{data}"}}]}])
...
def _github_request(path: str,method: str="GET",body:dict|None=None)->dict|list:
    ...
    req=Request("https://api.github.com"+path,data=data,method=method,headers={"Authorization":f"Bearer {token}","Accept":"application/vnd.github+json","User-Agent":"ALVIS","Content-Type":"application/json"})
    with urlopen(req,timeout=25) as response: return json.loads(response.read().decode("utf-8"))
...
def github_file(repo: str,path: str,ref: str="")->str:
    suffix=f"?ref={quote(ref)}" if ref else ""; data=_github_request(f"/repos/{repo.strip()}/contents/{path.lstrip('/')}{suffix}")
    if isinstance(data,list): return json.dumps(data[:100],ensure_ascii=False,indent=2)
    return base64.b64decode(data["content"]).decode("utf-8",errors="replace")


def github_write_file(repo: str,path: str,content: str,message: str,branch: str="main")->str:
    """Commit a text-file change directly to a GitHub branch. Use only when the user explicitly asks ALVIS to modify GitHub."""
    encoded=base64.b64encode(content.encode("utf-8")).decode("ascii")
    sha=""
    try:
        existing=_github_request(f"/repos/{repo.strip()}/contents/{path.lstrip('/')}?ref={quote(branch)}")
        if isinstance(existing,dict): sha=existing.get("sha","")
    except Exception:
        pass
    body={"message":message,"content":encoded,"branch":branch}
    if sha: body["sha"]=sha
    result=_github_request(f"/repos/{repo.strip()}/contents/{path.lstrip('/')}","PUT",body)
    return f"Committed {path} to {repo}:{branch}\nCommit: {result.get('commit',{}).get('html_url','')}"


def github_issues(repo: str,state: str="open")->str:
    data=_github_request(f"/repos/{repo.strip()}/issues?state={quote(state)}&per_page=30")
    return "\n".join(f"#{x['number']} {x['title']} — {x['html_url']}" for x in data if "pull_request" not in x)


def github_actions(repo: str,limit: int=10)->str:
    data=_github_request(f"/repos/{repo.strip()}/actions/runs?per_page={max(1,min(int(limit or 10),30))}")
    return "\n".join(f"#{x['run_number']} {x['name']} | {x['status']} | {x.get('conclusion')} | {x['html_url']}" for x in data.get("workflow_runs",[]))
...
"type_text":type_text,"hotkey":hotkey,"run_command":run_command,"search_web":search_web,"fetch_url":fetch_url,"extract_links":extract_links,
"github_repo":github_repo,"github_file":github_file,"github_write_file":github_write_file,"github_issues":github_issues,"github_actions":github_actions,
}
...
_schema("list_files","List workspace files.",{"path":{"type":"string"}}),_schema("system_info","Get Windows system information",{}),_schema("list_windows","List visible Windows application windows",{}),
_schema("inspect_window","Inspect visible Windows UI controls",{"title":{"type":"string"}},["title"]),_schema("click_control","Click a named visible Windows control",{"title":{"type":"string"},"control_text":{"type":"string"}},["title","control_text"]),
_schema("screenshot","Take a screenshot of the desktop",{"path":{"type":"string"}}),_schema("vision_screenshot","Take/read a screenshot with AI vision",{"path":{"type":"string"},"prompt":{"type":"string"}}),
_schema("mouse_move","Move mouse to screen coordinates",{"x":{"type":"integer"},"y":{"type":"integer"}},["x","y"]),_schema("mouse_click","Click mouse at screen coordinates",{"x":{"type":"integer"},"y":{"type":"integer"},"button":{"type":"string"}},["x","y"]),
_schema("type_text","Paste Unicode text into focused control",{"text":{"type":"string"}},["text"]),_schema("hotkey","Press keyboard shortcut",{"keys":{"type":"string"}},["keys"]),
_schema("run_command","Run a development/build command; destructive commands are blocked",{"command":{"type":"string"},"cwd":{"type":"string"}},["command"]),
_schema("search_web","Search public web for current information",{"query":{"type":"string"},"max_results":{"type":"integer"}},["query"]),_schema("fetch_url","Read a public web page",{"url":{"type":"string"}},["url"]),_schema("extract_links","Extract links from a public web page",{"url":{"type":"string"}},["url"]),
_schema("github_repo","Inspect a GitHub repository",{"repo":{"type":"string"}},["repo"]),_schema("github_file","Read a GitHub repository file",{"repo":{"type":"string"},"path":{"type":"string"},"ref":{"type":"string"}},["repo","path"]),
_schema("github_write_file","Commit a requested text-file change to GitHub",{"repo":{"type":"string"},"path":{"type":"string"},"content":{"type":"string"},"message":{"type":"string"},"branch":{"type":"string"}},["repo","path","content","message"]),
_schema("github_issues","List GitHub repository issues",{"repo":{"type":"string"},"state":{"type":"string"}},["repo"]),_schema("github_actions","List recent GitHub Actions builds",{"repo":{"type":"string"},"limit":{"type":"integer"}},["repo"]),
]
