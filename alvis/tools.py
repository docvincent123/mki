...
from .config import WORKSPACE, get_github_token
...
def open_app(app: str) -> str:
    ...
    if not exe:
        raise ToolError("Application name is empty")
...
def type_text(text: str, interval: float = 0.01) -> str:
    import pyautogui
    pyautogui.write(text, interval=max(0, min(interval, 0.2)))
    return "Typed text into the focused control"
...
def run_command(command: str, cwd: str = ".") -> str:
    """Run a development command. Potentially destructive commands are blocked for confirmation in chat."""
    ...
    if dangerous.search(command):
        return "NEEDS_CONFIRMATION: potentially destructive command blocked. Ask the user to confirm before running it."
    p = _safe_path(cwd)
...
def github_file(repo: str, path: str, ref: str = "") -> str:
    ...
    data = _github_request(f"/repos/{repo.strip()}/contents/{path.lstrip('/')}{suffix}")
...
TOOLS: dict[str, Callable[..., Any]] = {
    ...
    "list_files": list_files, "system_info": system_info, "list_windows": list_windows,
    "inspect_window": inspect_window, "click_control": click_control, "screenshot": screenshot,
    "mouse_move": mouse_move, "mouse_click": mouse_click, "type_text": type_text, "hotkey": hotkey,
    "run_command": run_command, "search_web": search_web, "fetch_url": fetch_url, "extract_links": extract_links,
    "github_repo": github_repo, "github_file": github_file, "github_issues": github_issues,
}
...
SCHEMAS = [
    ...
    _schema("list_files", "List workspace files.", {"path":{"type":"string"}}),
    _schema("system_info", "Get Windows system information.", {}),
    _schema("list_windows", "List visible Windows application windows.", {}),
    _schema("inspect_window", "Inspect visible Windows UI controls.", {"title":{"type":"string"}}, ["title"]),
    ...
    _schema("type_text", "Type text into the currently focused Windows control.", {"text":{"type":"string"},"interval":{"type":"number"}}, ["text"]),
    ...
    _schema("fetch_url", "Read a public web page.", {"url":{"type":"string"}}, ["url"]),
    _schema("extract_links", "Extract links from a public web page.", {"url":{"type":"string"}}, ["url"]),
    _schema("github_repo", "Inspect a GitHub repository.", {"repo":{"type":"string"}}, ["repo"]),
    _schema("github_file", "Read a file from a GitHub repository.", {"repo":{"type":"string"},"path":{"type":"string"},"ref":{"type":"string"}}, ["repo","path"]),
